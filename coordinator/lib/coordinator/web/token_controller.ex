defmodule Coordinator.Web.TokenController do
  ...
  defp page(conn, tokens, new_token) do
    ...
    <nav><a href="/admin/dashboard">Dashboard →</a><a href="/admin/workers">Workers →</a><a href="/admin/oban">Oban dashboard →</a><a href="/auth/logout">Log out (#{esc(conn.assigns[:current_admin])})</a></nav>
    #{reveal(new_token)}
    <h2>Issue a key</h2>
